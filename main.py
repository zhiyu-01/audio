...
class Audiowave:
    ...
    def __init__(self):
        ...
        np.array(self.x)
        np.array(self.y)
        self.path = "E:\python\wav\test.wav"
        ...
        self.wf1 = wave.open(r"E:\python\wav\test1.wav", 'wb')
    ...
    def wavehex_to_DEC(self, wavedata, wavewidth, wavechannel):
        ...
        x = np.linspace(0, len(self.Timedata[0]) - 1,
                        len(self.Timedata[0])) / self.framerate
    ...
    def wavedraw1(self, filedir):
        ...
        timedata = self.wavehex_to_DEC(self.wavedata, self.wavewidth,
                                       self.wavechannel)
    ...
    def Dynamic_run(self, filedir):  #
        ...
        self.Timedata = \
        self.wavehex_to_DEC(self.wavedata, self.wavewidth, self.wavechannel)[1][
            0]
        print(len(self.Timedata))
        temp = list(np.linspace(0, int(self.nframes / self.N),
                                int(self.nframes / self.N) + 1))
        # temp.pop()
        ani = FuncAnimation(self.fig, self.Dynamic_updata, frames=temp,
                            interval=self.N / self.framerate * 1000,
                            init_func=self.Dynamic_init, blit=True)
        plt.show()

    def wavehex_to_DEC_n(self, wavedata, wavewidth,
                         wavechannel):  # 录音存储数据十六进制数据转换十进制
        # print("#####################")
        Timedata = []
        ...
        x = np.linspace(0, len(Timedata[0]) - 1,
                        len(Timedata[0])) / self.framerate
    ...
    def Dynamic_micwave_update(self, n):  # 动态显示图像-更新图像
        ...
        # self.c+=d
        # print(self.c)
        x, y = self.wavehex_to_DEC_n(self.data, self.wavewidth,
                                     self.wavechannel)
    ...
    def Dynamic_micwave_run(self):  # 动态显示图像
        ...
        ani = FuncAnimation(self.fig, self.Dynamic_micwave_update,
                            ...
                            init_func=self.Dynamic_micwave_init, blit=True)
        plt.show()

    def filter(self, data, m, n):
        x, DEC_data = self.wavehex_to_DEC_n(data, self.wavewidth,
                                            self.wavechannel)
...
# a=Audiowave().wavedrawall(r"E:\pythonProject2\wav\P月23日 下午1点03分.wav")
...
if __name__ == '__main__':
    t1 = threading.Thread(target=recorder_run,
